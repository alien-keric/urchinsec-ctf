...
def is_valid_file(parser, arg):
    if not os.path.exists(arg):
        parser.error("The file does not exist: %s" % arg)
    else:
        return arg
...
def main(captureFile, tshark):
    if tshark is None:
        tshark = 'tshark'
    os.system(f'{tshark} -r {captureFile} -e usb.capdata -T fields -Y "(usb.urb_type == URB_COMPLETE)" > {tmp_file}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--file', 
                        dest='captureFile', 
                        help='Path to the capture file (.pcapng)', 
                        metavar="FILE", 
